...
5. **Finding 2, item 3** — the remaining feature handoff (`mediaGen` video
   stubs), one session. Item 2 (`videoTransform`) is ✅ **done 2026-08-30**;
   note its reported premise was wrong — the `default:` branch was unreachable
   and the inert nodes were hiding in *named* cases. Worth re-checking the
   other entries on this list against the code before trusting their wording.
