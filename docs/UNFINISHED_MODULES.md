...
| ~~9~~ | ~~`biomedical`~~ | **DONE in S174.** Removed read-path `ensureBootstrapped` from `dashboard()` (pure read), made `avgTurnaroundMin` `number | null` (null until a study completes) with `provenance` block, removed `org-windels` defaults on all 10 routes via `orgOf(req,res)` 403 + `authenticate`, catalogued `bm:img/imgs/ph/phs/tl/tls/ops/meta` org_scoped, added `/app/biomedical` console (6 tabs, null-aware TAT as "— not measured") + expanded web client (10 methods), fixed PlatformPage TAT `||0` lie, 14 new unit tests + 8 e2e, updated defect-pinning assertion. |
| ~~10~~ | ~~`healthEcosystem`~~ | **DONE in S175.** Removed read-path `ensureBootstrapped` from `dashboard(oid,userId)` (pure read, `hasData:false` when empty, no `hec:*` write), eliminated `userId ?? "anon"` fallback (now 403), made `dashboard(oid: string, userId: string)` require both, added `orgOf`/`userOf` 403 on all 22 routes + `authenticate`, catalogued `hec:meta/profile/metrics/sessions/meds/notes/alerts/wearables/devices/vaccines/screenings` org_scoped, added `/app/health-ecosystem` console (7 tabs, honest empty banner, three-bucket badges) + expanded web client (22 methods). 14 new unit tests + 9 e2e; existing zero-aggregate test preserved via `hasData` flag. |
| ~~11~~ | ~~`opex`~~ | **DONE in S176.** Removed `ensureBootstrapped` from `createAlert` and `dashboard` (both now pure, no `opex:<org>:meta` write on read/write), made `ensureBootstrapped(logger, oid: string)` require `oid` (no `"org-windels"`), added `orgOf(req)` 403 on all 3 legacy routes (consistent with `opexAssurance` sub-router, `authenticate` already at router level). No new catalog entries (already `opex` + `opx:*` org_scoped). No new console (one already honors `provenance` + nullable `GET /opex/trust`). 8 new unit tests (no-meta, require-oid, isolation). |
| ~~12~~ | ~~`cognitive`~~ | **DONE in S177.** Removed read-path `ensureBootstrapped` from `dashboard()` (pure read, no `cog:meta` write), widened nine structural fields (`selfEvolutionHealth/autoFixes30d/dnaCompleteness/marketplaceUnifiedAssets/federationPartners/innovationProposalsOpen/innovationPipelineValueUsd/civilizationEntities/worldScenariosTracked`) to `number | null` (`null` until built) with `provenance` (`structural_null`), made `ensureBootstrapped(logger, oid: string)` require `oid`, made `orgOf` guard `AppError.forbidden`, updated PlatformPage `CognitiveTab` to `metric()` (— not `0%`). 7 new unit tests. |
| ~~13~~ | ~~`command`~~ | **DONE in S178.** Removed read-path `ensureBootstrapped` from `CommandService.dashboard()` (pure read, no `cmd:meta` write, no cascade into `CommandOperationsService`), made `ensureBootstrapped(logger, oid: string)` require `oid` (no `"org-windels"`), updated `registerCommandRoutes` `orgOf`/`userOf` to `AppError.forbidden` guards (consistent with `opexAssurance`, `authenticate` already at router level). No new catalog entries (already `cmd:*`). 7 new unit tests. |
| ~~14~~ | ~~`disasterRecovery`~~ | **DONE in S179.** Removed read-path `ensureBootstrapped` (dashboard pure read, no `dr:*` write), made `DrDashboard.activeRegion: string → string | null` (`null` until configured), `replicationLagMs: number → number | null`, `components:[]` when none, `provenance` (`configured`/`unconfigured`), made `ensureBootstrapped(logger, oid?: string)` require `oid`, removed `oid = "org-windels"` defaults on all 8 service methods, updated `registerDisasterRecoveryRoutes` to `authenticate` + `orgOf` 403 on all 13 handlers. Catalogued `dr:active/status/ev/drill/drills/em/m/notes` org_scoped. Updated PlatformPage to “— no topology” when null. 9 new unit tests. |
| ~~15~~ | ~~`benchmarks`~~ | **DONE in S180.** Removed read-adjacent `ensureBootstrapped` seed (`bm:m:org {0,0}`) — dashboard now returns honest `0`s via `hgetall` fallback with no `bm:*` write, made `ensureBootstrapped(logger, oid?: string)` require `oid`, removed `oid="org-windels"` / `organizationId || "org-windels"` defaults on `dashboard/runBenchmark/schedule/listRuns`, updated `registerBenchmarksRoutes` to `authenticate` + `orgOf` 403 on all 4 handlers. Catalogued `bm:run/runs/m/area/sched/scheds/notes` org_scoped. 8 new unit tests. |
