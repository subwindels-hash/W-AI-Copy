...
- **115** API test files; **39** Playwright specs in `tests/e2e/`

## 4. The session-by-session arc (S1 → S123)
...
3. Run `corepack pnpm install --frozen-lockfile && make verify`; record the
   ...
   (`node audit/build-inventory.mjs`, 2026-08-06): 106 modules — **103 COMPLETE,
   0 PARTIAL**, 2 STUB-by-design (`events`, `webhook`), 1 DEMO DATA (`quantum`).
   **The module-completion track is done: every implementable module is COMPLETE.**
   Remaining work: the runtime-validation track and the two by-design stubs.
4. Run the S1–S6 and S89–S123 runtime-validation tracks in a target
