...
- **112** API test files; **36** Playwright specs in `tests/e2e/`

## 4. The session-by-session arc (S1 → S120)
...
3. Run `corepack pnpm install --frozen-lockfile && make verify`; record the
   ...
   (`node audit/build-inventory.mjs`, 2026-08-06): 106 modules — 100 COMPLETE,
   3 PARTIAL, 2 STUB-by-design (`events`, `webhook`), 1 DEMO DATA (`quantum`).
   Remaining PARTIAL modules, in the one-by-one completion order:
   `sustainability`, `talk`, `usage`.
   **Next module to complete: `sustainability`.**
4. Run the S1–S6 and S89–S120 runtime-validation tracks in a target
