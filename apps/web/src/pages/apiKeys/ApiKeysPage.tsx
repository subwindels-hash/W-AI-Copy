...
import { Check, Clipboard, KeyRound, Plus, RefreshCw, ShieldCheck, Trash2 } from "lucide-react";
...
export function ApiKeysPage() {
  ...
  const [scopes, setScopes] = useState<AkScope[]>(["READ"]);
  ...
  async function create() {
    if (!name.trim()) return;
    try { setCreated(await apiKeysApi.create({ name: name.trim(), scopes, expiresInDays: expiresInDays ? Number(expiresInDays) : undefined })); setName(""); setExpiresInDays(""); setScopes(["READ"]); await load(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); }
  }
