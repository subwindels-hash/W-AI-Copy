...
export const DOC_SECTIONS: DocSection[] = [
  ...
  },
  {
    id: "sdk",
    title: "API Clients",
    description: "Call the key-authenticated REST gateway from any language.",
    blocks: [
      { type: "h2", text: "API Clients" },
      { type: "p", text: "There is no published WINDELS package on npm or PyPI. Integrate against the REST gateway at /api/rest/v1, which is authenticated with an API key and is the same surface the Developer Gateway console documents. Every endpoint below exists today." },
      { type: "h3", text: "Authentication" },
      { type: "p", text: "Create a key in Settings → API Keys, then send it as the X-Api-Key header. Keys carry scopes; a request missing a required scope is rejected with 403." },
      { type: "h3", text: "TypeScript / JavaScript" },
      { type: "code", lang: "ts", text: "const res = await fetch('https://your-host/api/rest/v1/workflows/wf_abc123/execute', {\n  method: 'POST',\n  headers: {\n    'X-Api-Key': process.env.WINDELS_API_KEY!,\n    'Content-Type': 'application/json',\n  },\n  body: JSON.stringify({ input: {} }),\n});\n\nconst { ok, data, error } = await res.json();\nif (!ok) throw new Error(error.message);\nconsole.log(data.status); // queued | running | succeeded | failed | canceled" },
      { type: "h3", text: "Python" },
      { type: "code", lang: "python", text: "import os, requests\n\nres = requests.post(\n    'https://your-host/api/rest/v1/workflows/wf_abc123/execute',\n    headers={'X-Api-Key': os.environ['WINDELS_API_KEY']},\n    json={'input': {}},\n)\nbody = res.json()\nif not body['ok']:\n    raise RuntimeError(body['error']['message'])\nprint(body['data']['status'])" },
      { type: "h3", text: "Available endpoints" },
      { type: "ul", items: [
        "POST /ai/complete — AI completion via the provider registry",
        "GET /agents — list agents available to the key's organization",
        "POST /agents/:id/execute — execute an agent with input/context",
        "POST /workflows/:id/execute — run a workflow; billed to the usage ledger",
        "GET /workflows/:id/runs — list recent runs of a workflow",
        "POST /workflows/:id/runs/:runId/cancel — cancel an in-flight run",
        "GET /knowledge/search — semantic knowledge-base search",
        "GET /trading/analysis — trading-intelligence instrument analysis",
        "POST /media/generate — generate image/audio/video media",
      ]},
      { type: "callout", tone: "info", text: "All responses share the {ok, data, error} envelope. Open the Developer Gateway console for the live scope requirements on each endpoint." },
    ],
  },
];
