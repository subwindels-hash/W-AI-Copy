import { api } from "./api";
import type { Constitution, ConstitutionDashboard, ConstitutionPolicy, ConstitutionRule, ConstitutionViolation, CheckResult } from "@windels/shared";
export type {
  Constitution, ConstitutionDashboard, ConstitutionPolicy, ConstitutionViolation, CheckResult,
  ConstitutionDomain, ConstitutionPolicyStatus,
  // S163 — structured rules and the fail-closed posture.
  ConstitutionRule, ConstitutionRuleKind, ConstitutionCheckPosture,
} from "@windels/shared";
...
export const constitutionApi = {
  ...
  upsertPolicy: (input: Partial<ConstitutionPolicy> & { domain: ConstitutionPolicy["domain"]; title: string; statement: string; enforcementLevel: ConstitutionPolicy["enforcementLevel"]; status: ConstitutionPolicy["status"]; rule?: ConstitutionRule }) =>
