 ...
 */
import { api } from "./api";
import type { EpDashboard, EpExpertAgent, EpCourse, EpExpertPackage } from "@windels/shared";
export type { EpDashboard, EpExpertAgent, EpCourse, EpExpertPackage } from "@windels/shared";

export const epApi = {
  dashboard: () => api<EpDashboard>("/experts/dashboard/rollup"),
  agents: () => api<EpExpertAgent[]>("/experts/agents"),
  query: (id: string, question: string) =>
    api<{ answer: string; disclaimer: string; citations?: string[] }>(`/experts/agents/${id}/query`, { method: "POST", json: { question } }),
