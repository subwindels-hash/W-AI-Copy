...
export const lotteryApi = {
  lotteries: () => api<LiDashboard["lotteries"]>("/lottery-intel/lotteries"),
  dashboard: (lotteryId?: string) => api<LiDashboard>("/lottery-intel/dashboard", { params: { lotteryId } }),
  rules: (lotteryId?: string) => api<LiDashboard["rules"]>("/lottery-intel/rules", { params: { lotteryId } }),
  ...
  draws: (lotteryId?: string) => api<LiDraw[]>("/lottery-intel/draws", { params: { lotteryId } }),
  draw: (id: string) => api<LiDraw>(`/lottery-intel/draws/${id}`),
  numbers: (params?: { lastN?: number; window?: string; lotteryId?: string }) => api<LiNumberStat[]>("/lottery-intel/numbers", { params }),
  stars: (params?: { lastN?: number; window?: string; lotteryId?: string }) => api<LiNumberStat[]>("/lottery-intel/stars", { params }),
  distribution: (params?: { lastN?: number; lotteryId?: string }) => api<LiDistributionSnapshot>("/lottery-intel/distribution", { params }),
  pairs: (kind?: "MAIN" | "BONUS", lotteryId?: string) => api<LiPairStat[]>("/lottery-intel/pairs", { params: { kind, lotteryId } }),
