...
export const AiEconomyService = {
  ...
  async dashboard(oid: string): Promise<EconomyDashboard> {
    ...
    return {
      creditsInCirculation: 0, creditsSpent30d: 0, creditsEarned30d: 0, computeRevenue30d: 0, computeCost30d: totalCost, marginPct: 0,
      gpuUtilizationPct: allocations.length ? Math.round(allocations.reduce((n, a) => n + a.utilizationPct, 0) / allocations.length) : 0,
      gpusAvailable: 0, gpusTotal: 0, activeAllocations: allocations.length, forecasts: [], topDepartments: [...departments.entries()].map(([department, d]) => ({ department, spend: Math.round(d.spend * 100) / 100, credits: d.credits })).sort((a, b) => b.spend - a.spend),
