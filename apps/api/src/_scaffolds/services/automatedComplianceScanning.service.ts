...
/**
 * ⚠️  S212 SAFETY GUARD — DO NOT REMOVE WITHOUT IMPLEMENTING THE CHECK.
 *
 * The four functions below (`checkPIIInLogs`, `checkEncryptionAtRest`,
 * `checkDataExport`, `checkDataErasure`) were written to `return violations`
 * where `violations` is an empty array that nothing ever pushes to. As written
 * they are a compliance scanner that **always reports "no violations"** — a
 * control that is far more dangerous than an absent one, because a passing
 * scan is evidence of nothing while looking like evidence of compliance.
 *
 * This file is unreachable scaffolding today. If it is ever revived, these
 * guards make the module fail loudly instead of silently certifying.
 */
function scaffoldNotImplemented(check: string): never {
  throw Object.assign(
    new Error(
      `COMPLIANCE CHECK NOT IMPLEMENTED — "${check}" has no implementation. ` +
      `It previously returned an empty violation list, which reads as "compliant". ` +
      `Implement the check before enabling this scanner.`,
    ),
    { code: "COMPLIANCE_CHECK_NOT_IMPLEMENTED", status: 501 },
  );
}
...
async function checkPIIInLogs(
  ...
  scaffoldNotImplemented("PII in logs");
...
async function checkEncryptionAtRest(
  ...
  scaffoldNotImplemented("encryption at rest");
...
async function checkDataExport(
  ...
  scaffoldNotImplemented("data export capability");
...
async function checkDataErasure(
  ...
  scaffoldNotImplemented("data erasure capability");
