...
export async function signPackage(
  ...
  /**
   * ⚠️  S212 SAFETY GUARD — DO NOT REMOVE WITHOUT IMPLEMENTING REAL SIGNING.
   *
   * This was `sha256(checksum + privateKey)`. That is a keyed hash, NOT a
   * digital signature: it is symmetric, so anyone who can verify can also
   * forge, and the "private key" is recoverable offline by brute force over a
   * weak secret. A package marked `signed: true` on this basis carries no
   * authenticity guarantee while appearing to.
   *
   * Use an asymmetric primitive (`crypto.sign`/`crypto.verify` with Ed25519 or
   * RSA-PSS) before enabling packaging.
   */
  throw Object.assign(
    new Error(
      "PACKAGE SIGNING NOT IMPLEMENTED — the previous implementation was a keyed hash, " +
      "not a signature, and provided no authenticity guarantee. Wire an asymmetric " +
      "signing key (Ed25519 / RSA-PSS) before signing packages.",
    ),
    { code: "SIGNING_NOT_IMPLEMENTED", status: 501 },
  );
...
export async function verifyPackageSignature(
  ...
  /**
   * ⚠️  S212 SAFETY GUARD — DO NOT REMOVE WITHOUT IMPLEMENTING REAL VERIFICATION.
   *
   * This recomputed `sha256(checksum + publicKey)` and compared it to the
   * stored signature. Two things are wrong: it is symmetric (so verification
   * is forgery), and it mixes in the PUBLIC key — a value any caller can
   * supply — so anyone able to call this function can mint a value that
   * "verifies". It also used `===`, which is not constant-time.
   */
  return {
    valid: false,
    error:
      "PACKAGE VERIFICATION NOT IMPLEMENTED — the previous implementation recomputed a hash " +
      "over the caller-supplied public key, so any package could be made to verify. " +
      "Wire asymmetric verification (crypto.verify) before trusting package signatures.",
  };
  // eslint-disable-next-line no-unreachable
...
async function calculateChecksum(fileUrl: string): Promise<string> {
  /**
   * ⚠️  S212 SAFETY GUARD — DO NOT REMOVE WITHOUT HASHING THE ACTUAL BYTES.
   *
   * This hashed the URL STRING, not the file it points at. The returned value
   * is stable for a given URL no matter how the artifact behind it changes, so
   * it detects neither corruption nor tampering — the two things a checksum
   * exists to detect — while every caller treats it as if it does.
   */
  throw Object.assign(
    new Error(
      `CHECKSUM NOT IMPLEMENTED — the previous implementation hashed the URL string ` +
      `("${fileUrl}") rather than the file contents, so it could not detect corruption ` +
      `or tampering. Download the artifact and hash its bytes before use.`,
    ),
    { code: "CHECKSUM_NOT_IMPLEMENTED", status: 501 },
  );
}
