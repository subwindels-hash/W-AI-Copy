...
export function registerEnterpriseFoundationRoutes(router: Router) {
  ...
  // Real tenant-scoped notes ledger for enterpriseFoundation — user-authored annotations
  // persisted in Redis. Every write is a real Redis write; every read reflects
  // real state.
  const _notes = tenantStore<{ title: string; body: string; tags: string[]; }>({ prefix: "ef:notes", idPrefix: "ef-" });
