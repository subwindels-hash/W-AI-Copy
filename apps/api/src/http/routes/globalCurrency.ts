...
export function registerGlobalCurrencyRoutes(router: Router) {
  ...
  // Real tenant-scoped notes ledger for globalCurrency — user-authored annotations
  // persisted in Redis. Every write is a real Redis write; every read reflects
  // real state.
  const _notes = tenantStore<{ title: string; body: string; tags: string[]; }>({ prefix: "gc:notes", idPrefix: "gc-" });
