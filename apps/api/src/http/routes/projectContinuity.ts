...
export function registerProjectContinuityRoutes(router: Router) {
  ...
  r.post("/:id/extract", validate({ params: z.object({ id: z.string().min(1).max(100) }) }), async (req, res, next) => {
  ...
  r.post("/:id/inventory", validate({ params: z.object({ id: z.string().min(1).max(100) }) }), async (req, res, next) => {
  ...
  r.post("/:id/verify", validate({ params: z.object({ id: z.string().min(1).max(100) }) }), async (req, res, next) => {
  ...
  r.post("/intake", multipartSingle("archive"), async (req, res, next) => {
    try {
