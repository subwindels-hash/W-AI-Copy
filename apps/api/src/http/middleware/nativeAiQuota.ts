...
export async function nativeAiQuota(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const product = await prisma.apiProduct.findFirst({ where: { organizationId: null, slug: "native-ai", enabled: true }, select: { id: true } });
    if (product) {
      ...
      if (subscription?.quota && subscription.usedThisMonth >= subscription.quota) {
        return res.status(429).json({ error: { message: "Native AI API monthly quota exceeded", type: "rate_limit_error", code: "quota_exceeded", param: null }, request_id: req.requestId });
      }
    }
    next();
