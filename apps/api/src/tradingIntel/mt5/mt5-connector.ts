...
export class Mt5Connector implements IBrokerConnector {
  ...
  async connect(accountId: string, creds: ConnectCredentials, opts: ConnectOptions): Promise<ConnectResult> {
    ...
    try {
      ...
      logger.info("[mt5] account connected", { accountId, server: creds.server, transport });
      ...
      logger.error("[mt5] connect failed", { err: e, accountId, server: creds.server, transport });
