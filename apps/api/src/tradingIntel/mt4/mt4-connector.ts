/**
 * WINDELS AI OS — MetaTrader 4 Broker Connector (IBrokerConnector).
 *
...
import { getZmqTransport, Mt5ZmqTransport } from "../mt5/mt5-zmq-transport.js";
import { getHttpTransport, Mt5HttpTransport } from "../mt5/mt5-http-transport.js";
import { getMetaApiTransport, Mt5MetaApiTransport } from "../mt5/mt5-metaapi-transport.js";
...
export class Mt4Connector implements IBrokerConnector {
  readonly broker = "mt4" as const;
  readonly label = "MetaTrader 4";
  ...
  async isAvailable(): Promise<boolean> {
    const hasBridge = !!(env.WINDELS_MT4_BRIDGE_ZMQ || env.WINDELS_MT4_BRIDGE_HTTP || env.WINDELS_MT5_BRIDGE_ZMQ || env.WINDELS_MT5_BRIDGE_HTTP);
    const hasMeta = !!(env.WINDELS_MT4_METAAPI_TOKEN || env.WINDELS_METAAPI_TOKEN);
  ...
  async initialize(): Promise<void> {
    ...
    logger.info("[mt4] connector initialized", { transports: this.supportedTransports });
  }

  async shutdown(): Promise<void> {
    for (const id of Array.from(this.accounts.keys())) {
      await this.disconnect(id).catch((e) => logger.warn("[mt4] disconnect during shutdown failed", { err: e, accountId: id }));
    }
  ...
  private emitState(acct: AccountRuntime, status: BrokerConnectionStatus, error?: string) {
    ...
    for (const h of this.stateHandlers) {
      try { h(acct.state.accountId, status, error); } catch (e) { logger.warn("[mt4] state handler threw", { err: e }); }
    }
  ...
  async connect(accountId: string, creds: ConnectCredentials, opts: ConnectOptions): Promise<ConnectResult> {
    ...
    try {
      ...
      handle.on("tick", (aid: string, tick: BrokerTick) => {
        ...
        for (const sub of a.tickSubscribers) { try { sub(aid, tick); } catch (e) { logger.warn("[mt4] tick subscriber threw", { err: e }); } }
  ...
  async disconnect(accountId: string): Promise<DisconnectResult> {
    ...
    try {
      ...
      logger.warn("[mt4] transport disconnect error", { err: e, accountId });
    }
  ...
  async sync(accountId: string, scope: { account?: boolean; symbols?: boolean; positions?: boolean; orders?: boolean; history?: boolean; historyDays?: number }): Promise<SyncResult> {
    ...
    try {
      ...
      logger.warn("[mt4] sync failure", { err: e, accountId, scope, consecutiveErrors: a.state.consecutiveErrors });
      return out;
    }
  }
  ...
  async sendOrder(accountId: string, req: BrokerOrderRequest): Promise<OrderResult> {
    ...
    if (a.opts.config?.readOnly || env.WINDELS_MT4_GLOBAL_READONLY) return { ok: false, error: "account is configured read-only; orders are not transmitted" };
  ...
  async subscribeTicks(accountId: string, symbols: string[], handler: TickHandler): Promise<{ subscribed: string[] }> {
    ...
    if (toSub.length) {
      try {
        ...
        logger.warn("[mt4] tick subscribe failed for some symbols", { err: e, symbols: toSub });
      }
    }
  ...
  private pickTransport(opts: ConnectOptions): ConnectorTransport {
    ...
    if (opts.config?.metaapiToken || (env.WINDELS_MT4_METAAPI_TOKEN || env.WINDELS_METAAPI_TOKEN)) return "metaapi_cloud";
  ...
  private buildHandle(transport: ConnectorTransport, opts: ConnectOptions): Mt5ZmqTransport | Mt5HttpTransport | Mt5MetaApiTransport {
    switch (transport) {
      ...
      case "metaapi_cloud": {
        const token = opts.config?.metaapiToken ?? (env.WINDELS_MT4_METAAPI_TOKEN || env.WINDELS_METAAPI_TOKEN);
