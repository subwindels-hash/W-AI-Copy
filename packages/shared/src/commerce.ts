...
export interface CommerceCart { id: string; userId: string; organizationId: string; items: CommerceCartItem[]; subtotalCents: number; currency: string; createdAt: string; updatedAt: string; }
export interface CommerceProduct { id: string; name: string; description?: string; priceCents: number; currency: string; stockQuantity: number; images?: string[]; category?: string; attributes?: Record<string, unknown>; isActive: boolean; }
export interface CommerceOrderItem { productId: string; name: string; quantity: number; unitPriceCents: number; totalPriceCents: number; }
export interface CommerceOrder { id: string; userId: string; organizationId: string; items: CommerceOrderItem[]; subtotalCents: number; taxCents?: number; shippingCents?: number; totalCents: number; status: CommerceOrderStatus; paymentStatus: CommercePaymentStatus; shippingAddress?: Record<string, unknown>; billingAddress?: Record<string, unknown>; createdAt: string; updatedAt: string; }
export interface CommerceDashboard { totalOrders: number; totalRevenueCents: number; avgOrderValueCents: number | null; ordersByStatus: Record<string, number>; }
