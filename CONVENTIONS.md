...
## Demo-data elimination pass (Sessions 1–88+, 2026-07-31) — Decisions Logged

- **No `Math.random` in routed session modules, enforced:** the guard test `noRandomData.guard.test.ts` walks `src/**` and fails on any `Math.random(` outside an explicit allowlist (request-id hex, WebRTC tokens, echo-provider jitter, the user-facing random tool, tradingIntel synthetic-candle noise, the project-intake pattern matcher). "Seeded demo data" is now deterministic constants, never per-request fabricated metrics.
- **Deterministic seeds ≠ real data:** converting seeds to fixed constants removes the *randomness* but not the *demo-ness* — dashboards that aggregate real persisted records (created via write endpoints) are the honest source; deterministic seeds only provide baseline reference content for the seeded super-admin. Modules with write paths were left aggregating real records; modules without them show stable reference values.
- **Health data is the strictest case:** a fresh health user gets zeros + empty lists + `riskFlags` derived from real alerts — never fabricated clinically-validated biometrics. Daily wellness scores are computed from recorded metrics (sleep/stress/hydration windows), 0 when none.
- **Simulation timing is out of scope of the guard's spirit:** `setTimeout` sleeps that model async work (e.g. DR drill restore, update deploy) stay, but their *results* are deterministic; random failure probabilities (`Math.random() > 0.05`) were removed in favor of deterministic pass or real validation checks.
