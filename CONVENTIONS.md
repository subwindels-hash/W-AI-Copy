...
- **In-Sandbox Verification Status (`🟡 VERIFIED (partial)`).** Because native Prisma query engine downloads (`binaries.prisma.sh`) and live PostgreSQL 17 / Redis 8 are network-blocked in-sandbox, all sessions remain recorded as `🟡 VERIFIED (partial)` until executed in a target deployment environment.
