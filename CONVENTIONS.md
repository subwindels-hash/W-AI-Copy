...
### Session 198 — scanner `web.pages` is router-grounded

1. **A directory is not a page.** The S191 `findWebPages()` checked `pages/<dir>/` existence against a hand-maintained `PAGE_HOME` map, so a one-file re-export nothing imports counted exactly as much as a routed console: seven dead alias dirs (`pages/businessIntelligence/`, `enterpriseSearch/`, `enterpriseFinOps/`, `mediaFactory/`, `promptTemplates/`, `apiKeys/`, `canvasCollab/`) were live inventory findings, `engineering` was credited with the `events`/`webhook` consoles, and deleting a route changed nothing. The scanner now parses `apps/web/src/router.tsx` (the thing that makes a page reachable) and attributes a page to a module only when a rendering route path matches the module's key/kebab/`moduleRoutePrefix`/`ROUTE_ALIASES` token, or when the page imports the module's resolved web client. Unrouted bindings (`dashboard/AdminDashboard`) and unrouted alias dirs no longer count. Adding a `pages/<moduleKey>/` directory alone will **not** satisfy the inventory anymore — the page must be routed (or import the module's client), which is what S182–S190 meant to measure all along.
2. **A shared shell is not a console.** `admin/PlatformPage.tsx` (a tab per module since S176–S190) and the generic `lib/api.ts` fetch wrapper are deliberately excluded from client attribution — matching them reported the shell to ~40 modules and two dozen pages to `auth`. A module's console is its own routed page.
3. **The router itself was broken.** A second `const AgentsPage` (marketing, `pages/marketing/MarketingPages`) collided with the app console binding — a duplicate-const `SyntaxError` that failed the entire web build and had silently pointed `/app/workforce` ("Workforce Hub") at a marketing page. The marketing binding is `MarketingAgentsPage` now. When touching `router.tsx`, keep component names unique: the scanner's router table (and the build) both assume one binding per component.




### Session 199 — dead alias-dir cleanup + full language library, translation & detection

1. **Six dead alias page dirs were deleted; `apiKeys/` was not one of them.** S198 listed seven never-imported `pages/<key>/` alias dirs. Six of them (`businessIntelligence/`, `enterpriseSearch/`, `enterpriseFinOps/`, `mediaFactory/`, `promptTemplates/`, `canvasCollab/`) really were one-line re-exports nothing imports — the router lazy-imports the real consoles from their canonical dirs (`bi/`, `search/`, `finops/`, `media/`, `admin/`, `canvas/`), so deleting the alias dirs is inert. The seventh, `pages/apiKeys/ApiKeysPage.tsx`, was mislabeled: it is the **real 12 KB** ApiKeys implementation that `pages/apikey/ApiKeyPage.tsx` and `pages/developerPortal/DeveloperPortalPage.tsx` import. Deleting it would break the web build, so it stays. The S198 scanner comment was corrected. The router-grounded inventory is unaffected — all seven modules still report a page via their routed canonical consoles.
2. **The language catalog is the single source of truth, and it is generated.** The ~250-language library lives in `apps/api/src/languageLearning/catalog.data.ts`, emitted from `scripts/gen/genLanguageCatalog.mjs`. Add/rename/flag a language by editing the generator data and re-running it — never by scattering literals across routes/UI. `registry.ts` turns each row into an `LlLanguage`, adds `TRANSLATION`/`LANGUAGE_DETECTION` to every entry, and sets `learningSupported` only where `curriculum.ts` authors a real pack. Regional/script variants are first-class distinct entries (`zh-Hans`/`zh-Hant`, `pt-BR`/`pt-PT`, `pa`/`pa-Arab`, `ms`/`ms-Arab`, `sat`/`sat-Latn`, `tzm`/`zgh`, `iu`/`iu-Latn`, `crh-Cyrl`/`crh-Latn`, `kmr`/`ckb`, `fr-CA`), each carrying its own `bcp47`, `writingSystem` and `textDirection`. `getPack` aliases the two variant codes that reuse a base curriculum (`pt-BR`→`pt`, `zh-Hans`→`zh`).
3. **Translation and detection go through the AI fabric and never lie.** `languageLearning/translate.ts` calls `aiRegistry.complete()` (JSON mode) with a translator-grade system prompt that preserves the target regional/script variant/locale, formality, formatting, idioms and RTL direction — not word-for-word substitution. Detection runs an offline Unicode-script pass first (decisive for unique scripts, e.g. kana⇒Japanese), then the model, validating any returned code against the registry so a hallucinated code cannot leak. The engine source is always reported (`REAL` vs `DEMO`); on provider failure or an unparseable/empty result the service throws a typed error (`TRANSLATION_PROVIDER_UNAVAILABLE` / `TRANSLATION_FAILED`) — it **never** silently returns the input or a guess. Routes: `POST /language-learning/detect`, `POST /language-learning/translate`. Web: a searchable `LanguagePicker` (search / detect / recents / favorites / swap / native names / RTL / keyboard nav) and a `TranslateView` tab on the Language Teacher page. `catalog()` reports translation-only languages honestly instead of throwing when no curriculum pack exists.
